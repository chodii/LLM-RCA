...
def ensure_within_dir(base_dir: Path, target: Path) -> None:
    ...
    if target != base_dir and base_dir not in target.parents:
        raise RuntimeError(f"Unsafe archive path detected: {target} is outside {base_dir}")


def safe_extract_zip(src: Path, out_dir: Path) -> None:
    with zipfile.ZipFile(src, "r") as zf:
        for member in zf.infolist():
            member_path = Path(member.filename)
            if member_path.is_absolute() or ".." in member_path.parts:
                raise RuntimeError(f"Unsafe path in zip archive: {member.filename}")
            ensure_within_dir(out_dir, out_dir / member.filename)

        zf.extractall(out_dir)


def safe_extract_tar(src: Path, out_dir: Path) -> None:
    with tarfile.open(src, "r:*") as tf:
        for member in tf.getmembers():
            ...
            ensure_within_dir(out_dir, out_dir / member.name)

        tf.extractall(out_dir)


def safe_extract_7z(src: Path, out_dir: Path) -> None:
    try:
    ...
    with py7zr.SevenZipFile(src, mode="r") as zf:
        for name in zf.getnames():
            ...
            ensure_within_dir(out_dir, out_dir / name)
...
def process_round(in_dir: Path, out_dir: Path, manifest_dir: Path, start_counter: int) -> int:
    ...
    with open(manifest_path, "w", encoding="utf-8") as mf:
        ...
        for src in discovered_inputs:
            ...
            with tempfile.TemporaryDirectory(prefix="extract_round_") as tmp:
                ...
                try:
                ...
                except INVALID_ARCHIVE_ERRORS as e:
                    dst = namer.reserve(src.name)
                    out_size = safe_copy_file(src, dst)
                    copied_outputs += 1
                    invalid_archive_outputs += 1
                    bytes_out += out_size

                    record = {
                        "type": "invalid_archive_copied",
                        "input": str(src),
                        "input_size": src_size,
                    ...
                    mf.write(json.dumps(record) + "\n")
                    print(
                        "WARNING: could not extract archive-like file; "
                        f"copied it instead: {src} ({e.__class__.__name__}: {e})"
                    )
                    continue
    ...
    summary = {
        ...
        "invalid_archive_outputs": invalid_archive_outputs,
