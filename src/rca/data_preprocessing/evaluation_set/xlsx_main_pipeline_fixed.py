# -*- coding: utf-8 -*-
"""
Main pipeline for matching source JSON timestamps to XLSX rows.

This version does:
1) date-only overlap between source JSON timestamps and Excel datetimes
2) score each candidate row by how many source log lines it matches
3) keep the best row, or all tied best rows, for each source timestamp item

Expected companion files in the same folder:
- xlsx_extract_dates_rows.py
- xlsx_compare_dates.py
- xlsx_extract_matched_rows.py

Usage:
    python ./xlsx_main_pipeline_best_match.py ^
        "C:/path/to/file.xlsx" ^
...
Optional:
    python ./xlsx_main_pipeline_best_match.py ^
        "C:/path/to/file.xlsx" ^
        "C:/path/to/source_dates.json" ^
        --positions-out xlsx_date_positions.json ^
        --matches-out json_date_matches.json ^
        --hist-out json_dates_histogram.pdf ^
        --rows-out matched_rows_candidates.json ^
        --final-out source_date_row_matches_best.json ^
        --include-empty ^
        --min-score 1 ^
        --keep-ties
"""
...
def main():
    ...
    mod_extract_rows = load_module(base_dir / "xlsx_extract_matched_rows.py", "xlsx_extract_matched_rows")

    xlsx_path = str(Path(args.xlsx_path).resolve())
    source_json = str(Path(args.source_json).resolve())
    positions_out = str(Path(args.positions_out).resolve())
    matches_out = str(Path(args.matches_out).resolve())
    hist_out = str(Path(args.hist_out).resolve())
    rows_out = str(Path(args.rows_out).resolve())
    final_out = str(Path(args.final_out).resolve())
