...
def api(xlsx_path, source_json
        , positions_out="xlsx_date_positions.json"
        , matches_out="json_date_matches.json"
        , hist_out="json_dates_histogram.pdf"
        , rows_out="matched_rows_candidates.json"
        , final_out="source_date_row_matches_best.json"
        , include_empty=False, min_score=1, keep_ties = False):
    base_dir = Path(__file__).resolve().parent
    out_dir = base_dir / "out"
    (out_dir).mkdir(parents=True, exist_ok=True)
    final_out = abs_out_file(out_dir, final_out)
    if source_json is None or xlsx_path is None:
        if not os.path.exists(final_out):
            raise Exception("Error: the "+str(final_out)+" does not exits")
        return final_out
    ...
    positions_out = str((out_dir / positions_out).resolve()) if not Path(positions_out).is_absolute() else str((out_dir / Path(positions_out)).resolve())
    matches_out = str((out_dir / matches_out).resolve()) if not Path(matches_out).is_absolute() else str((out_dir / Path(matches_out)).resolve())
    hist_out = str((out_dir / hist_out).resolve()) if not Path(hist_out).is_absolute() else str((out_dir / Path(hist_out)).resolve())
    rows_out = str((out_dir / rows_out).resolve()) if not Path(rows_out).is_absolute() else str((out_dir / Path(rows_out)).resolve())
