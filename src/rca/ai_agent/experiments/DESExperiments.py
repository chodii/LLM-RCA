...
def api(context_manager, incidents_json, chunk_size ,model, VALIDATION=False):
    ...
    for i, incident in enumerate(IssueLoader.load_incidents(incidents_json=incidents_json
        ...
        rca, retrieved, usages, _convers = DESAI.api(user_problem=user_problem
                           , context_manager=context_manager
                           , model=model)
        rca = rca["content"]
        logger.log(incident=incident, rca=rca, retrieved=retrieved, conversation=_convers, usages=usages)
        evaluator.evaluate(incident, rca, retrieved, usages[0])
        # log: rca, retrieved, summaries                        RDY
        # log also: reference & grounded part of reference      
        # log meta: conv. rounds, total cost, len (retrieved X summaries X final RCA)
